...
from python.ingestion_function.src.lambda_utils import (
    get_timestamp,
    write_timestamp,
    collect_table_data,
...
DB_USERNAME = secrets_manager_client.get_secret_value(SecretId="totesys-username")["SecretString"]
DB_PASSWORD = secrets_manager_client.get_secret_value(SecretId="totesys-password")["SecretString"]
DB_HOST = secrets_manager_client.get_secret_value(SecretId="totesys-hostname")["SecretString"]
DB_PORT = secrets_manager_client.get_secret_value(SecretId="totesys-port")["SecretString"]
DB_NAME = secrets_manager_client.get_secret_value(SecretId="totesys-database")["SecretString"]
...
def lambda_handler(event, context):
    for table in tables:
        ...
        try:
            ...
            return {
                'statusCode': 500,
                'body': f"Error 500 Server Error: {e}"
            }
        else:
