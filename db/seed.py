...
def seed(address, counterparty, currency, department, design, payment_type, payment, purchase_order, sales_order, staff, transaction):
...
def insert_address_data(address):
    ...
    values = ', '.join([f"""({literal(addr['address_id'])}, 
...
def insert_counterparty_data(counterparty):
    ...
    values = ', '.join([f"""({literal(cpty['counterparty_id'])}, 
...
def insert_currency_data(currency):
    ...
    values = ', '.join([f"""({literal(curr['currency_id'])}, 
...
def insert_department_data(departments):
    ...
    values = ', '.join([f"""({literal(dept['department_id'])}, 
...
def insert_design_data(design):
    ...
    values = ', '.join([f"""({literal(design['design_id'])}, 
...
def insert_payment_type_data(payment_types):
    ...
    values = ', '.join([f"""({literal(ptype['payment_type_id'])}, 
...
def insert_staff_data(staff_records):
    ...
    values = ', '.join([f"""({literal(staff['staff_id'])}, 
...
def insert_sales_order_data(sales_order):
    ...
    values = ', '.join([f"""({literal(order['sales_order_id'])}, 
...
def insert_purchase_order_data(purchase_orders):
    ...
    item_quantity, item_unit_price, currency_id, agreed_delivery_date, agreed_payment_date, 
    agreed_delivery_location_id)
    VALUES """

    values = ', '.join([f"""({literal(po['purchase_order_id'])}, 
...
def insert_transaction_data(transactions):
    ...
    values = ', '.join([f"""({literal(txn['transaction_id'])}, 
...
def insert_payment_data(payments):
    ...
    values = ', '.join([f"""({literal(payment['payment_id'])}, 
