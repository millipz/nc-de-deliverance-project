...
data "archive_file" "loading_lambda_package" {
  type        = "zip"
  source_dir = "${path.module}/../python/src/loading_function"
...
data "template_file" "step-function-template" {
    template = file("step_function.tpl")
    vars = {
        env = var.env_name
    }
}